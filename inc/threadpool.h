 ...
 *
 * @brief A simple but powerful C++17 thread pool class. Please visit the GitHub
 * repository at https://github.com/bshoshany/thread-pool for documentation and
...
/**
 * @brief A simple but powerful thread pool class. Maintains a queue of tasks,
 * which are executed by threads in the pool as they become available.
 */
class thread_pool {
...
public:
   ...
   *
   * @param _thread_count The number of threads to use. Default value is the
   ...
   * CPU cores. If the argument is zero, 1 thread will be used.
   */
  thread_pool(const ui32 &_thread_count = std::thread::hardware_concurrency())
      : thread_count(std::max<ui32>(_thread_count, 1)),
        threads(new std::thread[std::max<ui32>(_thread_count, 1)]) {
    create_threads();
  }

  /**
   * @brief Destruct the thread pool. Waits for all submitted tasks to be
   * completed, then destroys all threads.
   */
  ~thread_pool() {
    wait_for_tasks();
    running = false;
   ...
   * executing. The loop will be equivalent to "for (T i = first_index; i <=
   * last_index; i++) loop(i);".
   *
   * @tparam T The type of the loop index. Should be a signed or unsigned
   * integer.
   * @tparam F The type of the function to loop through.
   * @param first_index The first index in the loop (inclusive).
   * @param last_index The last index in the loop (inclusive).
   * @param loop The function to loop through. Should take exactly one argument,
   * the loop index.
   * @param num_tasks The maximum number of tasks to split the loop into.
   * Default is to use the number of threads in the pool.
   */
  template <typename T, typename F>
  void parallelize_loop(T first_index, T last_index, const F &loop,
                        ui32 num_tasks = 0) {
    if (num_tasks == 0)
    ...
    for (ui32 t = 0; t < num_tasks; t++) {
      ...
      T end = (t == num_tasks - 1)
                  ? last_index
                  : (T)((t + 1) * block_size + first_index - 1);
      std::cout << start << '-' << end << '\n';
      blocks_running++;
  ...
  /**
   * @brief Reset the number of threads in the pool. Waits for all submitted
   * tasks to be completed, then destroys all threads and creates a new thread
   * pool with the new number of threads.
   *
   * @param _thread_count The number of threads to use. Default value is the
   * total number of hardware threads available, as reported by the
   * implementation. With a hyperthreaded CPU, this will be twice the number of
  ...
  /**
   * @brief Wait for all submitted tasks to be completed - both those that are
   * currently being executed by threads, and those that are still waiting in
   * the queue. To wait for a specific task, use push_task_future instead.
   */
  void wait_for_tasks() {
...
private:
  ...
  /**
   * @brief A worker function to be assigned to each thread in the pool. Pops
   * tasks out of the queue and executes them, until the atomic variable running
   * is set to false.
   */
  void worker() {
    while (running) {
  ...
  /**
   * @brief An atomic variable to keep track of how many tasks are currently
   * waiting to finish - either still in the queue, or running in a thread.
   */
...
/**
 * @brief A class to synchronize printing to an output stream by different
 * threads.
 */
class synced_stream {
public:
   ...
   * other threads print to this stream simultaneously, as long as they all use
   * this synced_stream object to print.
   *
   ...
   * simultaneously, as long as they all use this synced_stream object to print.
   *
   * @tparam T The types of the items
   * @param items The items to print.
   */
